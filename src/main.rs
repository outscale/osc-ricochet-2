...
fn have_request_filter(
    filter: &json::JsonValue,
    ...
    fn nested_comp_filter(
        i: usize,
        nested_resources: &Vec<&str>,
        resource: &JsonValue,
        needle: &JsonValue,
    ) -> bool {
        if i == nested_resources.len() - 1 {
            ...
            match nested_resources[i] {
                "*" => resource
                    .members()
                    .any(|el| nested_comp_filter(i + 1, nested_resources, el, needle)),
...
fn conflict(
    ...
    json["Errors"] = json::array![json::object! {Details: error, Code: code}];
    Err((
        jsonobj_to_strret(json, req_id),
...
fn bad_auth(error: String) -> Result<Response<Body>, Infallible> {
    ...
    let mut json_error = json::object! {
      Errors: [
        {
          Code: "1",
          Details: "error",
    ...
    json_error["Errors"][0]["Details"] = json::JsonValue::String(error);

    response
        .headers_mut()
        .append("WWW-Authenticate", "Basic".parse().unwrap());
    response
        .headers_mut()
        .append("Content-Type", "application/json".parse().unwrap());
    *response.body_mut() = Body::from(json::stringify_pretty(json_error, 2));
...
fn try_conver_response(
    rres: Result<(String, StatusCode), (String, StatusCode)>,
    need_convert: bool,
...
fn get_default_subregion(cfg: &JsonValue) -> String {
    match cfg.has_key("region") {
        true => match cfg["region"].has_key("subregions") {
            true => format!(
                "{}{}",
                cfg["region"]["name"], cfg["region"]["subregions"][0]
            ),
...
impl RicCall {
    ...
    fn eval(
        ...
        fn update_state(
            resources: &mut json::JsonValue,
            f: impl Fn(&json::JsonValue) -> bool,
            to_state: &str,
        ) {
        ...
        macro_rules! add_security_group {
            ($in_json:expr, $req_id:expr, $resource:expr) => {
                for sg_id in $in_json["SecurityGroupIds"].members() {
                    let name = match main_json[user_id]["SecurityGroups"]
                        .members_mut()
                        .find(|sg| *sg_id == sg["SecurityGroupId"])
                    {
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                format!("can't find SG id {}", sg_id).as_str(),
                            )
                        }
                    };
                ...
                for sg_name in $in_json["SecurityGroups"].members() {
                    let id = match main_json[user_id]["SecurityGroups"]
                        .members_mut()
                        .find(|sg| *sg_name == sg["SecurityGroupName"])
                    {
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                format!("can't find SG named {}", sg_name).as_str(),
                            )
                        }
                    };
                    $resource["SecurityGroups"]
        ...
        macro_rules! get_by_id_2 {
            ($resource_type:expr, $id_name:expr, $id:expr, $then_err:expr) => {{
                match main_json[user_id][$resource_type]
                    .members()
                    .position(|m| m[$id_name] == $id)
                {
        ...
        macro_rules! get_by_id {
            ($resource_type:expr, $id_name:expr, $id:expr) => {{
                get_by_id_2!(
                    ...
                    $id_name,
                    $id,
                    Err(bad_argument(req_id, json.clone(), "Element id not found"))
                )
            }};
        }
        ...
        macro_rules! require_arg_2 {
            ($json:expr, $req_id:expr, $in_json:expr, $arg:expr) => {{
                match $in_json.has_key($arg) {
                    true => $in_json[$arg].clone(),
                    _ => {
                        return bad_argument($req_id, $json, format!("{} required", $arg).as_str())
                    }
                }
            }};
        }
            ...
            ($nic:expr, $device_number:expr) => {{
                if $nic.has_key("LinkNic") {
                    return bad_argument(
                        req_id,
                        json,
                        format!("Nic id {} is already linked", $nic["NicId"]).as_str(),
                    );
                }
                ...
                for k in main_json[user_id][concat!(stringify!($resource), "s")].members() {
                    if k[concat!(stringify!($resource), "Name")] == $to_check {
                        return conflict(
                            req_id,
                            $json,
                            concat!(stringify!($resource), " Name conflict"),
                            "9000",
                        );
                    }
                }
        ...
        macro_rules! update_vm_nic_creation {
            ($vm:expr, $nic:expr) => {{
                ...
                if let Some(pip) = $nic["PrivateIps"]
                    .members()
                    .find(|pip| pip["IsPrimary"].as_bool().unwrap())
                {
        ...
        macro_rules! check_aksk_auth {
            ($auth:expr) => {{
                if $auth != AuthType::AkSk {
                    return eval_bad_auth(
                        req_id,
                        json,
                        format!("{:?} requires v4 signature", *self).as_str(),
                    );
                }
            }};
        }
        ...
        match *self {
            RicCall::Root => Ok(("Try POSTing to /ReadVms".to_string(), StatusCode::OK)),
            RicCall::Debug => {
            ...
            RicCall::ReadVms => {
                ...
                for i in rm_array {
                    ...
                    for (fgpu_idx, fgpu) in main_json[user_id]["FlexibleGpus"]
                        .clone()
                        .members()
                        .enumerate()
                    {
                    ...
                    for id in vms_ids.members() {
                        if *id == vm["VmId"] {
                            if vm["DeletionProtection"] == true {
                                return bad(req_id, json, "", 8018, "OperationNotSupported");
                            }
                            need_rm = true;
                        }
                    }
                ...
                let (lb_idx, lb) = match get_by_id!("LoadBalancers", "LoadBalancerName", lb_name) {
                    ...
                    Err(_) => {
                        return bad_argument(
                            req_id,
                            json,
                            format!("LoadBalancer `{}` not found", lb_name).as_str(),
                        )
                    }
                };
            ...
            RicCall::CreateLoadBalancer => {
                    ...
                    && lb["LoadBalancerType"] != "internal"
                {
                    return bad_argument(
                        req_id,
                        json,
                        "Type of load balancer must be internet-facing or internal",
                    );
                }
                for l in lb["Listeners"].members_mut() {
                    require_arg!(l, "BackendPort");
                    l["BackendProtocol"] =
                        optional_arg!(l, "BackendProtocol", l["LoadBalancerProtocol"].clone());
                ...
                if in_json.has_key("Subnets") {
                    ...
                    for subnet in lb["Subnets"].members() {
                        match get_by_id!("Subnets", "SubnetId", *subnet) {
                            Ok((t, idx)) => {
                                let curr_net =
                                    main_json[user_id][t][idx]["NetId"].as_str().unwrap();
                                if net_id.is_empty() {
                                    net_id = curr_net;
                                    ...
                                    return bad_argument(
                                        req_id,
                                        json,
                                        "Subnets must be in the same net",
                                    );
                                }
                            }
                            Err(_) => {
                                return bad_argument(
                                    req_id,
                                    json,
                                    format!("Subnet {} not found", subnet).as_str(),
                                )
                            }
                        }
                    }
                    ...
                    for sg in lb["SecurityGroups"].members() {
                        if get_by_id!("SecurityGroups", "SecurityGroupId", *sg).is_err() {
                            return bad_argument(
                                req_id,
                                json,
                                format!("Security Group {} not found", sg).as_str(),
                            );
                        }
                    }
                } else if !lb["Subnets"].is_empty() {
                    let sg = main_json[user_id]["SecurityGroups"]
                        .members()
                        .find(|sg| {
                ...
                if in_json.has_key("PublicIp") {
                    if lb["LoadBalancerType"] != "internet-facing" {
                        return bad_argument(
                            req_id,
                            json,
                            "Public IP should be specified with an internet-facing's LBU",
                        );
                    }
                    if !in_json.has_key("Subnets") {
                        return bad_argument(
                            req_id,
                            json,
                            "An internet-facing's LBU needs the Subnets parameter",
                        );
                    }
                    lb["PublicIp"] = in_json["PublicIp"].clone();
                    let subnet_idx = get_by_id!("Subnets", "SubnetId", lb["Subnets"][0])
                        .unwrap()
                        .1;
                    let subnet_st: Ipv4Net = main_json[user_id]["Subnets"][subnet_idx]["IpRange"]
                        .as_str()
                        .unwrap()
                        .parse()
                        .unwrap();
                    let used_ips =
                        used_ips_of_subnet!(&main_json[user_id]["Subnets"][subnet_idx]["SubnetId"]);
                    let private_ip = match subnet_st.hosts().find(|ip| {
                        !used_ips
                            .members()
                            .any(|used_ip| used_ip.as_str().unwrap() == ip.to_string())
                    }) {
                        Some(pip) => pip,
                        _ => return bad_argument(req_id, json, "all private ips used"),
                    };
                    main_json[user_id]["Subnets"][subnet_idx]["AvailableIpsCount"] = (main_json
                        [user_id]["Subnets"][subnet_idx]["AvailableIpsCount"]
                        .as_usize()
                        .unwrap()
                        - 1)
                    .into();
                    ...
                    let pip = match get_by_id!("PublicIps", "PublicIp", in_json["PublicIp"]) {
                        ...
                        Err(_) => {
                            return bad_argument(
                                req_id,
                                json,
                                "PublicIp doesn't correspond to an existing Ip",
                            )
                        }
                    };
                ...
                main_json[user_id]["LoadBalancers"]
                    .push(lb.clone())
                    .unwrap();
                    ...
                    if ptl.has_key("Additions") {
                        ...
                        if addition.has_key("AccountIds") {
                            for aid in addition["AccountIds"].members() {
                                match image["PermissionsToLaunch"]["AccountIds"]
                                    .members()
                                    .find(|id| aid == *id)
                                {
                                    Some(_) => {}
                                    _ => image["PermissionsToLaunch"]["AccountIds"]
                                        .push(aid.clone())
                                        .unwrap(),
                                }
                            }
                    ...
                    if ptl.has_key("Removals") {
                        ...
                        if removal.has_key("AccountIds") {
                            for aid in removal["AccountIds"].members() {
                                if let Some((idx, _)) = image["PermissionsToLaunch"]["AccountIds"]
                                    .members()
                                    .enumerate()
                                    .find(|(_, id)| aid == *id)
                                {
            ...
            RicCall::CreateImageExportTask => {
                ...
                main_json[user_id]["ImageExportTasks"]
                    .push(iet.clone())
                    .unwrap();
            ...
            RicCall::CreateImage => {
                ...
                main_json[user_id]["Images"].push(image.clone()).unwrap();
                json["Image"] = image;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::CreateSubnet => {
                ...
                let net = match get_by_id!("Nets", "NetId", subnet["NetId"]) {
                    ...
                    _ => {
                        return bad_argument(
                            req_id,
                            json,
                            "NetId doesn't correspond to an existing Net",
                        )
                    }
                };
                ...
                if !net_st.contains(&subnet_st) {
                    return bad_argument(
                        req_id,
                        json,
                        "Subnet IP range needs to be a subset of the Net",
                    );
                }
                ...
                main_json[user_id]["Subnets"].push(subnet.clone()).unwrap();
                json["Subnet"] = subnet;
            ...
            RicCall::CreateNet => {
                    ...
                    match net_st {
                        Ok(range) => {
                            if range.prefix_len() > 29 {
                                return conflict(req_id, json, "iprange size is nope", "9050");
                            }
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                "you range is pure &@*$ i mean invalid",
                            )
                        }
                    }
                } else {
                ...
                main_json[user_id]["SecurityGroups"]
                    .push(create_security_group!(
                        Some(net["NetId"].clone()),
                        "default",
                        "default security group"
                    ))
                    .unwrap();
                main_json[user_id]["Nets"].push(net.clone()).unwrap();
                json["Net"] = net;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::DeleteRoute => {
            ...
            RicCall::CreateNatService => {
                ...
                let ip = match main_json[user_id]["PublicIps"]
                    .members()
                    .find(|ip| ip_id == ip["PublicIpId"])
                {
                    Some(ip) => ip,
                    _ => {
                        return bad_argument(
                            req_id,
                            json,
                            "PublicIpId doesn't correspond to an existing id",
                        )
                    }
                };
                ...
                main_json[user_id]["NatServices"]
                    .push(nat_service.clone())
                    .unwrap();
            ...
            RicCall::CreateSnapshot => {
                ...
                let snap = if in_json.has_key("VolumeId") {
                    ...
                    match get_by_id!("Volumes", "VolumeId", volume_id) {
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                format!("Volume {} not found", volume_id).as_str(),
                            )
                        }
                    }
                    ...
                    return bad_argument(
                        req_id,
                        json,
                        "CreateSnapshot require either VolumeId or SourceSnapshotId",
                    );
                };
            ...
            RicCall::CreateRoute => {
                        ...
                        if in_json.has_key($target) {
                            if target_count > 0 {
                                return bad_argument(
                                    req_id,
                                    json,
                                    "Only one target resource is allowed",
                                );
                            }
                            target_count += 1;
                                ...
                                Ok((_, id)) => {
                                    ...
                                    if $fun(&el) {
                                        ...
                                        return bad_argument(
                                            ...
                                            json,
                                            format!(
                                                "{} is not in the RouteTable's net or is invalid",
                                                el[$id_name]
                                            )
                                            .as_str(),
                                        );
                ...
                set_route!(
                    ...
                    "NatServices",
                    "NatServiceId",
                    |nat: &JsonValue| nat["NetId"] == net_id
                );
                set_route!(
                    "NetPeeringId",
                    "NetPeerings",
                    "NetPeeringId",
                    |net_p: &JsonValue| net_p["AccepterNet"]["NetId"] == net_id
                        || net_p["SourceNet"]["NetId"] == net_id
                );
                set_route!("NicId", "Nics", "NicId", |nic: &JsonValue| nic["NetId"]
                    == net_id);
                set_route!("VmId", "Vms", "VmId", |vm: &JsonValue| vm.has_key("Nics")
                    && vm["Nics"].len() == 1
                    && vm["NetId"] == net_id);
                if in_json.has_key("GatewayId") {
                    if in_json["GatewayId"].as_str().unwrap().starts_with("vgw-") {
                        set_route!(
                            ...
                            "VirtualGateways",
                            "VirtualGatewayId",
                            |gw: &JsonValue| gw["NetToVirtualGatewayLinks"]
                                .members()
                                .any(|link| link["NetId"] == net_id)
                        );
                    } else {
                        set_route!(
                            "GatewayId",
                ...
                if rt["Routes"].members().any(|route| {
                    route == &new_route
                        || route["DestinationIpRange"] == new_route["DestinationIpRange"]
                }) {
                    return bad_argument(
                        req_id,
                        json,
                        "Route already exists or DestinationIpRange in use",
                    );
                }
            ...
            RicCall::CreateRouteTable => {
                ...
                main_json[user_id]["RouteTables"].push(rt.clone()).unwrap();
                json["RouteTable"] = rt;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            ...
            RicCall::LinkRouteTable => {
                ...
                if subnet_net_id != route_table["NetId"] {
                    return bad_argument(
                        req_id,
                        json,
                        "The Subnet and the route table must be in the same Net.",
                    );
                }
                ...
                route_table["LinkRouteTables"]
                    .push(link_route_table.clone())
                    .unwrap();
            ...
            RicCall::UnlinkRouteTable => {
                ...
                let get_rt_link_idx = || -> Option<(usize, usize)> {
                    for (route_table_idx, route_table) in
                        main_json[user_id]["RouteTables"].members().enumerate()
                    {
                        if let Some(link_route_table_idx) = route_table["LinkRouteTables"]
                            .members()
                            .position(|link| link["LinkRouteTableId"] == link_route_table_id)
                        {
                ...
                let (route_table_idx, link_route_table_idx) = match get_rt_link_idx() {
                    ...
                    None => {
                        return bad_argument(
                            req_id,
                            json,
                            format!("can't find the link route table {}", link_route_table_id)
                                .as_str(),
                        )
                    }
                };

                main_json[user_id]["RouteTables"][route_table_idx]["LinkRouteTables"]
                    .array_remove(link_route_table_idx);
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            ...
            RicCall::CreateClientGateway => {
                ...
                main_json[user_id]["ClientGateways"]
                    .push(cg.clone())
                    .unwrap();
                    ...
                    filters_do!(
                        ...
                        |o: &json::JsonValue| {
                            ...
                            update_state(
                                user_cgs,
                                |cg| {
                                    cg["ClientGatewayId"] == o["ClientGatewayId"]
                                        && cg["State"] == "deleting"
                                },
                                "deleted",
                            )
                ...
                let igw = match get_by_id!("InternetServices", "InternetServiceId", id) {
                    ...
                    _ => {
                        return bad_argument(
                            req_id,
                            json,
                            "SecurityGroupId doesn't corespond to an existing id",
                        )
                    }
                };
            ...
            RicCall::LinkPublicIp => {
                ...
                let ip_id = if in_json.has_key("PublicIp") {
                    ...
                    match main_json[user_id]["PublicIps"]
                        .members()
                        .find(|ip| pub_ip == ip["PublicIp"])
                    {
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                "PublicIp doesn't corespond to an existing Ip",
                            )
                        }
                    }
                ...
                let ip_idx = match user["PublicIps"]
                    .members()
                    .position(|iwg| ip_id == iwg["PublicIpId"])
                {
                    Some(idx) => idx,
                    _ => {
                        return bad_argument(
            ...
            RicCall::UnlinkPublicIp => {
                ...
                if in_json.has_key("PublicIpId") {
                    ...
                    match user_iwgs.members_mut().find(|iwg| id == iwg["PublicIpId"]) {
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                "PublicIpId doesn't corespond to an existing id",
                            )
                        }
                    };
                    return bad_argument(
                        req_id,
                        json,
                        "Sorry but UnlinkPublicIp with PublicIpId not yet supported",
                    );
                } else {
                    let id = require_arg!(in_json, "LinkPublicIpId");
                    let user = &mut main_json[user_id];

                    let link_idx = match user["LinkPublicIps"]
                        .members()
                        .position(|iwg| id == iwg["LinkPublicIpId"])
                    {
                        Some(iwg) => iwg,
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                "UnlinkPublicIpId doesn't corespond to an existing id",
                            )
                        }
                    };
                    ...
                    if let Some(vm_idx) = user["Vms"].members().position(|vm| vm_id == vm["VmId"]) {
                        ...
                        user["Vms"][vm_idx]["PublicIp"] =
                            Ipv4Addr::from(rng.gen_range(0..u32::MAX))
                                .to_string()
                                .into();
                    }

                    if let Some(ip_idx) = user["PublicIps"]
                        .members()
                        .position(|pip| ip_id == pip["PublicIpId"])
                    {
            ...
            RicCall::DeleteNet => {
                ...
                array_remove!(sgs, |sg| sg["NetId"] == id
                    && sg["SecurityGroupName"] == "default");
            ...
            RicCall::ReadAccessKeys => {
                json["AccessKeys"] = json::array![json::object! {
                    State:"ACTIVE",
            ...
            RicCall::ReadPublicCatalog => {
                json["Catalogs"] = json::array![json::object! {
                    Entries: json::array![
                        json::object!{
                            Category: "Mecha",
            ...
            RicCall::ReadPublicIpRanges => {
                json["PublicIps"] = json::array!["43.41.44.22/24", "34.14.44.22/24"];

                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::ReadRegions => {
                json["Regions"] = json::array![json::object! {
            ...
            RicCall::ReadAccounts => {
                ...
                json["Accounts"] = json::array![json::object! {
                        City:"カカリコ",
                        CompanyName: "plouf",
                        Country: "ハイラル",
            ...
            RicCall::LinkVolume => {
                ...
                match get_by_id!("Vms", "VmId", vm_id) {
                    Ok((_, vm_idx)) => match get_by_id!("Volumes", "VolumeId", volume_id) {
                        Ok((_, vol_idx)) => {
                            ...
                            let vol_bsu =
                                &mut main_json[user_id]["Vms"][vm_idx]["BlockDeviceMappings"];
                            vol_bsu
                                .push(json::object! {
            ...
            RicCall::CreateVolume => {
                ...
                if !matches!(
                    vol["VolumeType"].as_str().unwrap(),
                    "io1" | "gp2" | "standard"
                ) {
                    return bad_argument(req_id, json, "Bad VolumeType");
                }
                main_json[user_id]["Volumes"].push(vol.clone()).unwrap();
                json["Volume"] = vol;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::ReadRouteTables => {
                ...
                if !bytes.is_empty() {
                    ...
                    filters_check!(
                        ...
                        "RouteTables",
                        (NetIds, "NetId"),
                        (LinkRouteTableMain, "LinkRouteTables.*.Main")
                    );
                } else {
            ...
            RicCall::CreateKeypair => {
                ...
                main_json[user_id]["Keypairs"].push(kp.clone()).unwrap();
                json["Keypair"] = kp;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
                ...
                if id == -1 {
                    ...
                    if name == -1 {
                        return bad_argument(
                            req_id,
                            json,
                            "either SecurityGroupId or SecurityGroupName is require",
                        );
                    }
            ...
            RicCall::DeleteSecurityGroupRule => {
                    ...
                    Some(s) => match s {
                        ...
                        _ => {
                            return bad_argument(
                                req_id,
                                json,
                                "The direction of the flow must be `Inbound` or `Outbound`.",
                            )
                        }
                    },
                ...
                ) -> Result<(String, hyper::StatusCode), (String, hyper::StatusCode)>
                {
                    ...
                    array_remove_2!(json, req_id, sg[flow_to_str!(flow)], |other_rule| {
                        is_same_rule(&new_rule, other_rule)
                    });
            ...
            RicCall::CreateDirectLink => {
                ...
                if dl["Bandwidth"] != "1Gbps" && dl["Bandwidth"] != "10Gbps" {
                    return bad_argument(
                        req_id,
                        json,
                        "Bandwidth need to be either '1Gbps' or '10Gbps'",
                    );
                }
                main_json[user_id]["DirectLinks"].push(dl.clone()).unwrap();
                json["DirectLink"] = dl;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::CreatePublicIp => {
                ...
                main_json[user_id]["PublicIps"].push(eip.clone()).unwrap();
                json["PublicIp"] = eip;
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::CreateInternetService => {
                ...
                main_json[user_id]["InternetServices"]
                    .push(igw.clone())
                    .unwrap();
            ...
            RicCall::CreateSecurityGroupRule => {
                ...
                ) -> Result<(String, hyper::StatusCode), (String, hyper::StatusCode)>
                {
                    ...
                    if sg[flow_to_str!(flow)]
                        .members()
                        .any(|other_rule| is_same_rule(&new_rule, other_rule))
                    {
            ...
            RicCall::CreateSecurityGroup => {
                ...
                let net_id = if in_json.has_key("NetId") {
            ...
            RicCall::UpdateVm => {
                ...
                let vm_id = require_arg!(in_json, "VmId");
                json["ricochet-info"] =
                    format!("vm id: {}, but update vm barly implemented", vm_id).into();
                ...
                if in_json.has_key("VmInitiatedShutdownBehavior") {
                    vm["VmInitiatedShutdownBehavior"] =
                        in_json["VmInitiatedShutdownBehavior"].clone()
                }
            ...
            RicCall::CreateVms => {
                ...
                let mut vol = json::array![json::object! {
                        VolumeId: format!("vol-{:08x}", req_id),
                        Tags: [],
                        VolumeType: "standard",
                        SubregionName: get_default_subregion(&cfg),
                ...
                if in_json.has_key("SubnetId") {
                    if in_json.has_key("Nics") {
                        return bad_argument(
                            req_id,
                            json,
                            "can't have the `Nics` parameter if you specify a `SubnetId`",
                        );
                    }
                ...
                if in_json.has_key("Nics") {
                    ...
                    for nic_for_vm_creation in in_json["Nics"].members() {
                        if nic_for_vm_creation.has_key("NicId") {
                            let nic =
                                match get_by_id!("Nics", "NicId", nic_for_vm_creation["NicId"]) {
                                    ...
                                    Err(_) => {
                                        return bad_argument(
                                            ...
                                            json,
                                            format!(
                                                "can't find the Nic id {}",
                                                nic_for_vm_creation["NicId"]
                                            )
                                            .as_str(),
                                        )
                                    }
                            ...
                            let device_number = require_arg!(nic_for_vm_creation, "DeviceNumber")
                                .as_i32()
                                .unwrap();
                            if !(0..=7).contains(&device_number) {
                                return bad_argument(
                                    req_id,
                                    json,
                                    "the device number should be between 0 and 7, both included",
                                );
                            }
                            let mut nic = create_nic!(nic_for_vm_creation);
                            update_vm_nic_creation!(vm, nic);
                            link_nic!(nic, device_number);
                            nic["LinkNic"]["DeleteOnVmDeletion"] =
                                optional_arg!(nic_for_vm_creation, "DeleteOnVmDeletion", true);
                ...
                if in_json.has_key("SecurityGroupIds") || in_json.has_key("SecurityGroups") {
                    ...
                    let sg = main_json[user_id]["SecurityGroups"]
                        .members()
                        .find(|sg| {
                            sg["NetId"] == vm["NetId"] && sg["SecurityGroupName"] == "default"
                        })
                        .unwrap()
                        .clone();
                    vm["SecurityGroups"] = json::array![json::object! {
                ...
                vol.members()
                    .for_each(|v| main_json[user_id]["Volumes"].push(v.clone()).unwrap());
            ...
            RicCall::CreateTags | RicCall::DeleteTags => {
                ...
                if !in_json.has_key("Tags") && !in_json.has_key("ResourceIds") {
                    return bad_argument(
                        req_id,
                        json,
                        "CreateTags/DeleteTags require: ResourceIds, Tags argument",
                    );
                }
                ...
                let mut ids = Vec::new();
                let resources_todo = in_json["ResourceIds"]
                    .members()
                    .map(|id| {
                        ids.push(id.clone());
                        match id.as_str() {
                            Some(id) => match id.rsplit_once('-') {
                                ...
                                _ => Err(bad_argument(
                                    req_id,
                                    json.clone(),
                                    format!("invalide resource id {}", id).as_str(),
                                )),
                            },
                    ...
                    for tag in tags.members() {
                        ...
                        match *self {
                            RicCall::CreateTags => {
                                ...
                                user_main_json[resource_t][idx]["Tags"]
                                    .push((*tag).clone())
                                    .unwrap();
                            }
                            RicCall::DeleteTags => {
                                array_remove!(user_main_json["Tags"], |ot| ntag.eq(ot));
                                array_remove!(user_main_json[resource_t][idx]["Tags"], |ot| tag
                                    .eq(ot));
                            }
                            _ => todo!(),
                        };
            ...
            RicCall::ReadQuotas => {
                check_aksk_auth!(auth);
                json["QuotaTypes"] = json::array![json::object! {
                    Quotas: json::array![
                        json::object!{
            ...
            RicCall::CreateNetPeering => {
                ...
                let get_net_i = |net_id: &JsonValue| {
                    for (user_id, resources) in main_json.members().enumerate() {
                        if let Some(net) = resources["Nets"]
                            .members()
                            .find(|net| *net_id == net["NetId"])
                        {
                ...
                let (accepter_user_id, accepter_net) = match get_net_i(&accepter_net_id) {
                    ...
                    _ => {
                        return bad_argument(
                            ...
                            json,
                            format!(
                                "can't find user linked with accepter net id {}",
                                accepter_net_id
                            )
                            .as_str(),
                        )
                ...
                let (source_user_id, source_net) = match get_net_i(&source_net_id) {
                    ...
                    _ => {
                        return bad_argument(
                            ...
                            json,
                            format!(
                                "can't find user linked with source net id {}",
                                source_net_id
                            )
                            .as_str(),
                        )
                ...
                if source_user_id != user_id {
                    return bad_argument(
                        req_id,
                        json,
                        format!("the source net id {} needs to be your own", source_net_id)
                            .as_str(),
                    );
                }
                ...
                if main_json[source_user_id]["Vms"].is_empty()
                    || main_json[accepter_user_id]["Vms"].is_empty()
                {
                ...
                let get_net_peering = |source_net: &str, accepter_net: &str| {
                    main_json[source_user_id]["NetPeerings"]
                        .members()
                        .find(|net_p| {
                            source_net_id == net_p[source_net]["NetId"]
                                && accepter_net_id == net_p[accepter_net]["NetId"]
                        })
                };
                ...
                if let Some(reverse_net_peering) = get_net_peering("AccepterNet", "SourceNet") {
                    if reverse_net_peering["State"]["Name"].as_str().unwrap() == "active" {
                        ...
                        net_peering["State"]["Message"] =
                            "Rejected automatically because active reverse link already exists"
                                .into();
                    }
                }

                let source_net_ip_range: Ipv4Net =
                    source_net["IpRange"].as_str().unwrap().parse().unwrap();
                let accepter_net_ip_range: Ipv4Net =
                    accepter_net["IpRange"].as_str().unwrap().parse().unwrap();
                // When a net_peering failed, the result is sent back but not stored
                if source_net_ip_range.contains(&accepter_net_ip_range)
                    || accepter_net_ip_range.contains(&source_net_ip_range)
                {
                    net_peering["State"]["Name"] = "failed".into();
                    net_peering["State"]["Message"] =
                        "The two Nets must not have overlapping IP ranges".into();
                } else {
                    main_json[source_user_id]["NetPeerings"]
                        .push(net_peering.clone())
                        .unwrap();
                    if source_user_id != accepter_user_id {
                        main_json[accepter_user_id]["NetPeerings"]
                            .push(net_peering.clone())
                            .unwrap();
                    }
                }
            ...
            RicCall::ReadNetPeerings => {
                ...
                if !bytes.is_empty() {
                    ...
                    filters_check!(
                        ...
                        net_peerings,
                        "NetPeerings",
                        (SourceNetNetIds, "SourceNet.NetId"),
                        (AccepterNetNetIds, "AccepterNet.NetId")
                    );
                } else {
            ...
            RicCall::AcceptNetPeering => {
                ...
                let is_pending_net_p = |net_p: &JsonValue| {
                    net_peering_id == net_p["NetPeeringId"]
                        && format!("{:012x}", user_id)
                            == net_p["AccepterNet"]["AccountId"].as_str().unwrap()
                ...
                if !updated {
                    return bad_argument(
                        req_id,
                        json,
                        "can't find a net peering pending-acceptance",
                    );
                }
            ...
            RicCall::RejectNetPeering => {
                ...
                let net_peering =
                    match main_json[user_id]["NetPeerings"]
                        .members_mut()
                        .find(|net_p| {
                            net_p["NetPeeringId"] == net_peering_id
                                && net_p["State"]["Name"] == "pending-acceptance"
                        }) {
                        Some(net_p) => net_p,
                        None => {
                            return bad_argument(
                                ...
                                json,
                                format!(
                                    "can't find the net peering {} in pending-acceptance state",
                                    net_peering_id
                                )
                                .as_str(),
                            )
            ...
            RicCall::DeleteNetPeering => {
                ...
                let net_peering_id = require_arg!(in_json, "NetPeeringId");
                let is_request_owner = |net_peering: &JsonValue| {
                    net_peering["SourceNet"]["AccountId"].as_str().unwrap()
                        == format!("{:012x}", user_id)
                };
                let is_peer_net_owner = |net_peering: &JsonValue| {
                    net_peering["AccepterNet"]["AccountId"].as_str().unwrap()
                        == format!("{:012x}", user_id)
                };
                    ...
                    if resources["NetPeerings"].members().any(|net_peering| {
                        net_peering_id == net_peering["NetPeeringId"]
                            && (match net_peering["State"]["Name"].as_str().unwrap() {
                                ...
                                "active" => {
                                    is_request_owner(net_peering) || is_peer_net_owner(net_peering)
                                }
                                _ => false,
                            })
                    }) {
                        array_remove!(resources["NetPeerings"], |net_peering| net_peering_id
                            == net_peering["NetPeeringId"]);
                        deleted = true;
                    }
                }
                if !deleted {
                    return bad_argument(
                        req_id,
                        json,
                        format!("cannot delete the net peering {}", net_peering_id).as_str(),
                    );
                }
            ...
            RicCall::CreateVirtualGateway => {
                ...
                main_json[user_id]["VirtualGateways"]
                    .push(virtual_gateway.clone())
                    .unwrap();
            ...
            RicCall::ReadVirtualGateways => {
                ...
                if !bytes.is_empty() {
                    ...
                    filters_check!(
                        ...
                        virtual_gateways,
                        "VirtualGateways",
                        (LinkNetIds, "NetToVirtualGatewayLinks.*.NetId")
                    );
                } else {
            ...
            RicCall::LinkVirtualGateway => {
                ...
                let virtual_gateway =
                    match get_by_id!("VirtualGateways", "VirtualGatewayId", virtual_gateway_id) {
                ...
                if !virtual_gateway["NetToVirtualGatewayLinks"].is_empty() {
                    return bad_argument(
                        ...
                        json,
                        format!(
                            "Virtual Gateway {} is already linked to a Net",
                            virtual_gateway_id
                        )
                        .as_str(),
                    );
                ...
                virtual_gateway["NetToVirtualGatewayLinks"]
                    .push(virtual_gateway_link.clone())
                    .unwrap();
            ...
            RicCall::UnlinkVirtualGateway => {
                ...
                let get_vgw_link_idx = || -> Option<(usize, usize)> {
                    for (vgw_idx, vgw) in
                        main_json[user_id]["VirtualGateways"].members().enumerate()
                    {
                        if vgw["VirtualGatewayId"] == virtual_gateway_id {
                            for (link_idx, link) in
                                vgw["NetToVirtualGatewayLinks"].members().enumerate()
                            {
                ...
                let (vgw_idx, link_idx) = match get_vgw_link_idx() {
                    ...
                    None => {
                        return bad_argument(
                            req_id,
                            json,
                            format!("can't find link with net id {}", net_id).as_str(),
                        )
                    }
                };
                main_json[user_id]["VirtualGateways"][vgw_idx]["NetToVirtualGatewayLinks"]
                    .array_remove(link_idx);
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            RicCall::DeleteVirtualGateway => {
                ...
                let virtual_gateway_id = require_arg!(in_json, "VirtualGatewayId");
                array_remove!(virtual_gateways, |vgw| vgw["VirtualGatewayId"]
                    == virtual_gateway_id);
            ...
            RicCall::CreateNetAccessPoint => {
                ...
                if main_json[user_id]["NetAccessPoints"]
                    .members()
                    .any(|net_ap| {
                        net_ap["NetId"] == net_access_point["NetId"]
                            && net_ap["ServiceName"] == net_access_point["ServiceName"]
                    })
                {
                    return bad_argument(
                        req_id,
                        json,
                        format!(
                            "Net is already linked to the service `{}`",
                            net_access_point["ServiceName"]
                        )
                        .as_str(),
                    );
                }
                        ...
                        match get_by_id!("RouteTables", "RouteTableId", *rt_id) {
                            ...
                            Err(_) => {
                                return bad_argument(
                                    req_id,
                                    json,
                                    format!("can't find route table {}", rt_id).as_str(),
                                )
                            }
                            _ => {
                                return bad_argument(
                                    req_id,
                                    json,
                                    "route tables ids need to be exclusive",
                                )
                            }
                        }
                    }
                ...
                main_json[user_id]["NetAccessPoints"]
                    .push(net_access_point.clone())
                    .unwrap();
            ...
            RicCall::DeleteNetAccessPoint => {
                ...
                array_remove!(net_access_points, |net_ap| net_ap["NetAccessPointId"]
                    == net_access_point_id);
                Ok((jsonobj_to_strret(json, req_id), StatusCode::OK))
            }
            ...
            RicCall::LinkNic => {
                ...
                if vm["State"] != "running" && vm["State"] != "stopped" {
                    return bad_argument(
                        req_id,
                        json,
                        "the VM should be either `running` or `stopped`",
                    );
                }
                if vm.has_key("Nics") {
                ...
                if !(1..=7).contains(&device_number) {
                    return bad_argument(
                        req_id,
                        json,
                        "the device number should be between 1 and 7, both included",
                    );
                }
            ...
            RicCall::UnlinkNic => {
                ...
                let get_vm_niclink_idx = || -> Option<(usize, usize)> {
                    for (vm_idx, vm) in main_json[user_id]["Vms"].members().enumerate() {
                        if let Some(nic_idx) = vm["Nics"]
                            .members()
                            .position(|nic| nic["LinkNic"]["LinkNicId"] == linknic_id)
                        {
...
fn clasify_v4(userpass: &str) -> Option<(&str, String)> {
    ...
    Some((
        userpass
            .strip_prefix(format!("{}-HMAC-SHA256 ", which).as_str())
            .unwrap(),
        which,
    ))
}
...
async fn handler(
    ...
    if cfg["auth_type"] != "none" {
        ...
        if userpass.starts_with("Basic ") {
            ...
            let decoded = general_purpose::STANDARD.decode(based).unwrap();
            ...
            let cred = match cred.0.strip_prefix("Credential=") {
                ...
                _ => {
                    return bad_auth(
                        "\"Authorization Header is broken, should start witgh 'Credential='\""
                            .to_string(),
                    )
                }
            };
            let tuple_cred = match cred.split_once('/') {
                ...
                _ => {
                    return bad_auth(
                        "\"Authorization Header is broken, can't find ACCESS_KEY\"".to_string(),
                    )
                }
            };
            let ak = tuple_cred.0;
            ...
            match users.members().position(|u| {
                ...
                let x_date = match headers.get(which_v4_to_date(&which_v4)) {
                    Some(x_date) => x_date.to_str().unwrap().to_string(),
                    _ => {
                ...
                let tuple_cred = match cred.split_once(',') {
                    Some((_, sc)) => match sc.strip_prefix(" SignedHeaders=") {
                ...
                let mut canonical_hdrs = match signed_hdrs.contains("content-type") {
                    true => match headers.get("Content-Type") {
                        Some(ct) => format!("content-type:{}\n", ct.to_str().unwrap()),
                        _ => {
                            return v4_error_ret(
                                &mut error_msg,
                                "content-type found, but not found",
                            )
                        }
                    },
                    _ => "".to_string(),
...
{:x}",
                ...
                let credential_scope =
                    format!("{}/{}/{}/{}", short_date, region, api, "osc4_request");
                let mut hasher = Sha256::new();
...
{}
{:x}",
                    which_v4, x_date, credential_scope, canonical_request_sha
                );
                ...
                let mut hmac =
                    match HmacSha256::new_from_slice(format!("{}{}", which_v4, true_sk).as_bytes())
                    {
                        Ok(v) => v,
                        _ => return false,
                    };
                ...
                _ => {
                    ...
                    json_error["Errors"][0]["Details"] = json::JsonValue::String(error_msg);
    ...
    match to_call {
        Ok(which_call) => {
            let res = try_conver_response(
                which_call.eval(main_json, cfg, bytes, user_id, req_id, headers, auth),
                out_convertion,
            );
            let mut response = Response::new(Body::empty());
            if api == "directlink" {
                response
                    .headers_mut()
                    .append("x-amz-requestid", req_id.to_string().parse().unwrap());
            }
...
async fn main() {
    ...
    let usr_cfg_path = match args.len() {
        2 => args[1].clone(),
        _ => format!("{}/.osc/ricochet.json", env::var("HOME").unwrap()),
    };
    ...
    let any_server = match tls {
        true => match hyper_from_pem_files("cert.crt", "cert.key", Protocols::ALL, &addr) {
                    ...
                    .serve(make_service_fn(move |_| {
                        ...
                        async move {
                            ...
                            Ok::<_, hyper::Error>(service_fn(move |_req| {
                                ...
                                async move {
                                    log::info!("start processing: {:?}", _req);
                                    let resp = handler(_req, &connection, id, &cfg).await;
                                    log::info!("en processing: {:?}", resp);
                                    resp
                                }
                            }))
                        }
            ...
            Err(e) => {
                log::error!("failed to build tls listener: {:?}", e);
                return;
            }
        },
        false => {
